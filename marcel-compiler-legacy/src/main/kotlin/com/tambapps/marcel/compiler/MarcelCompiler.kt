...
class MarcelCompiler(compilerConfiguration: CompilerConfiguration): AbstractMarcelCompiler(compilerConfiguration) {
  ...
  @Throws(IOException::class, MarcelLexerException::class, MarcelParserLegacyException::class, MarcelSemanticLegacyException::class, MarcelCompilerException::class)
  ...
  fun compileSourceFiles(marcelClassLoader: MarcelClassLoader? = null, sourceFiles: Collection<SourceFile>, classConsumer: Consumer<CompiledClass>) {
    ...
    val asts = sourceFiles.map { sourceFile ->
      ...
      val parser = MarcelParserLegacy(typeResolver, sourceFile.className, tokens) //if (className != null) MarcelParser(typeResolver, className, tokens) else MarcelParser(typeResolver, tokens)
