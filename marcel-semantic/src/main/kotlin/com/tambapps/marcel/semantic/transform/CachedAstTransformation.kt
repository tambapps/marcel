        ...
        val ciaLambda = newLambda(classNode,
          ...
          val doComputeMethodCall = if (isMultiParams) {
            ...
            fCall(owner = outerRef(), method = doComputeMethod,
              arguments = doComputeMethod.parameters.mapIndexed { index, methodParameter -> cast(fCall(owner = ref(listLv), name = "get", arguments = listOf(int(index))), methodParameter.type) })
          }
          else fCall(owner = outerRef(), method = doComputeMethod, arguments = listOf(cast(argRef(0), cacheKeyRef.type)))
