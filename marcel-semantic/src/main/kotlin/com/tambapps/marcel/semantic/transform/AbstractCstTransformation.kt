...
abstract class AbstractCstTransformation : SyntaxTreeTransformation {
  ...
  private fun newScope(node: CstNode): ImportScope {
    ...
    while (cstNode !is SourceFileCstNode) {
      if (node.parent == null) return ImportScope(typeResolver, emptyList(), null)
      cstNode = node.parent!!
    }
    return ImportScope(typeResolver, ImportCstNodeConverter.convert(cstNode.imports), cstNode.packageName)
  }
