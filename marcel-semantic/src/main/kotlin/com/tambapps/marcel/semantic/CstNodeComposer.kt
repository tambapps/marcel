...
open class CstNodeComposer {
  ...
  protected fun methodNode(
    ...
    val methodNode = MethodCstNode(classNode, tokenStart, tokenEnd,
  ...
  protected fun type(type: JavaType): TypeCstNode = TypeCstNode(null, type.className, emptyList(), 0, LexToken.DUMMY, LexToken.DUMMY)
  ...
  protected inner class CstStatementsComposer(
    ...
    fun ifStmt(condition: ExpressionCstNode, trueStmt: StatementCstNode, falseStmt: StatementCstNode? = null,
               add: Boolean = true): IfStatementCstNode {
      val statement = IfStatementCstNode(condition, trueStmt, falseStmt, parent, LexToken.DUMMY, LexToken.DUMMY)
      if (add) statements.add(statement)
      return statement
    }

    fun ifStmt(condition: ExpressionCstNode,
               add: Boolean = true, trueStatementsComposerFunc: CstStatementsComposer.() -> Unit): IfStatementCstNode {
      ...
      val statement = IfStatementCstNode(condition,
