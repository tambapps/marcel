...
public interface Long2ObjectMap<V> extends Map<Long,V> {
	...
	default Set<Map.Entry<Long, V>> entrySet() {
		return (Set)long2ObjectEntrySet();
	}
	 ...
	 * map contains more than {@link Long#MAX_VALUE} elements, returns {@link Long#MAX_VALUE}.
	 *
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super V> consumer) {
	 final Iterator<Entry<V>> entrySet = iterator();
	 final Consumer<Long2ObjectMap.Entry <V> > wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V replace(final long key, final V value) { return containsKey(key) ? put(key, value) : null; }
