...
public interface Int2ObjectMap <V> extends Map<Integer,V> {
	...
	default Set<Map.Entry<Integer, V>> entrySet() {
		return (Set)int2ObjectEntrySet();
	}
	...
	default Iterator<Entry <V> > iterator() {
		return int2ObjectEntrySet().iterator();
	}
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super V> consumer) {
	 final Iterator<Entry<V>> entrySet = iterator();
	 final Consumer<Int2ObjectMap.Entry <V> > wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V replace(final int key, final V value) { return containsKey(key) ? put(key, value) : null; }
	 ...
	 * @since 8.0.0
	 */
	default V computeIfPresent(final int key, final java.util.function.BiFunction<? super Integer, ? super V, ? extends V> remappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default V compute(final int key, final java.util.function.BiFunction<? super Integer, ? super V, ? extends V> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final V oldValue = get(key);
	 ...
	 * @since 8.0.0
	 */
	default V merge(final int key, final V value, final java.util.function.BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
