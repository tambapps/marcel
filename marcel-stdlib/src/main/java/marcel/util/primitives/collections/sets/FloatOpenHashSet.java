...
import marcel.util.primitives.Hash;
...
import marcel.util.primitives.iterators.FloatIterators;
...
public class FloatOpenHashSet extends AbstractFloatSet implements java.io.Serializable, Cloneable, Hash {
	 ...
	 */
	public static FloatOpenHashSet of(final float e) {
	 FloatOpenHashSet result = new FloatOpenHashSet (1, DEFAULT_LOAD_FACTOR);
	 result.add(e);
	 return result;
	}
	/** Creates a new hash set with {@link Hash#DEFAULT_LOAD_FACTOR} as load factor
	 ...
	 */
	public static FloatOpenHashSet of(final float e0, final float e1) {
	 FloatOpenHashSet result = new FloatOpenHashSet (2, DEFAULT_LOAD_FACTOR);
	 result.add(e0);
	 if (!result.add(e1)) {
	 ...
	 * @throws IllegalArgumentException if there were duplicate entries.
	 */
	public static FloatOpenHashSet of(final float e0, final float e1, final float e2) {
	 FloatOpenHashSet result = new FloatOpenHashSet (3, DEFAULT_LOAD_FACTOR);
	 result.add(e0);
	 if (!result.add(e1)) {
	  throw new IllegalArgumentException("Duplicate element: " + e1);
	 }
	 ...
	 */
	public static FloatOpenHashSet of(final float... a) {
	 FloatOpenHashSet result = new FloatOpenHashSet (a.length, DEFAULT_LOAD_FACTOR);
	 for (float element : a) {
	...
	public boolean add(final float k) {
	 ...
	 if (( Float.floatToIntBits(k) == 0 )) {
	  ...
	  // The starting point.
	  if (! ( Float.floatToIntBits(curr = key[pos = Hash.mix( Hash.float2int(k) ) & mask]) == 0 )) {
	...
	protected final void shiftKeys(int pos) {
	 ...
	 for(;;) {
	  ...
	  for(;;) {
	   ...
	   slot = Hash.mix( Hash.float2int(curr) ) & mask;
	...
	public boolean remove(final float k) {
	 ...
	 int pos;
	 // The starting point.
	 if (( Float.floatToIntBits(curr = key[pos = Hash.mix( Hash.float2int(k) ) & mask]) == 0 )) return false;
	...
	protected void rehash(final int newN) {
	 ...
	 for(int j = realSize(); j-- != 0;) {
	  ...
	  if (! ( Float.floatToIntBits(newKey[pos = Hash.mix( Hash.float2int(key[i]) ) & mask]) == 0 ))
